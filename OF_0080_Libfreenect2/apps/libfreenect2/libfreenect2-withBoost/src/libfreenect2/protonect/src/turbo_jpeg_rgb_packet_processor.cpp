...
//        impl_->startTiming();
        ...
        //ofBufferToFile("test.jpg", tmp, true);
      
        if( ofLoadImage(pix, tmp) ){
        
            cout << " image loaded " << endl; 
////            impl_->frame->data = new unsigned char[ 3 * pix.getWidth() * pix.getHeight()];
////        
//              memcpy(impl_->frame->data, pix.getPixels(), 3 * pix.getWidth() * pix.getHeight() );
//
