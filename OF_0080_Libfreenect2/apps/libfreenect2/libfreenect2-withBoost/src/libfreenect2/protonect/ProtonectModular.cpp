...
int runKinect2(std::string binpath)
{
...
//  }
  ...
  rgb_bulk_transfers = libfreenect2::usb::BulkTransferPool(handle, 0x83);
  ...
  depth_iso_transfers = libfreenect2::usb::IsoTransferPool(handle, 0x84);
  ...
  printf("             speed: %s\n", speed_name[r]);
    ...
    cout << "RunKinect Completed" << endl;
}


void updateKinect(){
    cout << "updateKinect started" << endl;
...
//    cv::waitKey(1);
    ...
    cout << "updateKinect completed" << endl; 
}

void closeKinect(){
  rgb_bulk_transfersPtr->disableSubmission();
  depth_iso_transfersPtr->disableSubmission();

  CloseKinect(handle);

  rgb_bulk_transfersPtr->cancel();
  depth_iso_transfersPtr->cancel();

  // wait for all transfers to cancel
  // TODO: better implementation
  boost::this_thread::sleep(boost::posix_time::seconds(2));

  rgb_bulk_transfersPtr->deallocate();
  depth_iso_transfersPtr->deallocate();
